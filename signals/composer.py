...
def compose_bl_inputs(prices):
    ...
    if is_time_machine_mode:
        print("   -> [TIME MACHINE] Bypassing Live API (Macro/Skew/FX) for historical backtest.")
        ...
        print(f"2️⃣  Put-call volatility skew ({Q_WEIGHTS['skew'] * 100}% weight)")
        ...
        try:
            ...
            if isinstance(fx_data, pd.DataFrame):
                recent_fx = fx_data.iloc[:, 0].tail(5)
            else:
    ...
    print(f"3️⃣  Fundamental & Smart money ({Q_WEIGHTS['fundamental'] * 100}% weight)")
    Q_fundamental = generate_fundamental_views(tickers)

    print(f"4️⃣  Idiosyncratic alpha ({Q_WEIGHTS['alpha'] * 100}% weight)")
    try:
    ...
    final_raw_q = (Q_blended * Q_WEIGHTS['trend']).add(Q_skew * Q_WEIGHTS['skew'], fill_value=0) \
    ...
    for ticker in final_q_views.index:
        ...
        if clean_t in DEFENSIVE_ETFS:
            if not combined_kill_switch:
                ...
                if not is_time_machine_mode: print(f"   [Risk-On] {ticker} (ETF): Apply -2.0")
            else:
                # Risk-Off
                final_q_views[ticker] = 2.0
                if not is_time_machine_mode: print(f"   🚨 [Risk-Off] {ticker} (ETF): Apply +2.0")
    print("============================================================\n")
    ...
    if combined_kill_switch:
        if not is_time_machine_mode: print(f"🚨🚨 [DANGER] Kill-Switch ACTIVATED!")
        if not is_time_machine_mode and vix_trigger: print(f"   - Cause: High VIX ({vix_level:.2f})")
        if not is_time_machine_mode and fx_trigger: print(f"   - Cause: FX Volatility ({fx_volatility*100:.2f}%)")
        if not is_time_machine_mode: print("   >>> Strategy: Moving to Cash/Safe-Haven mode.")
    else:
        if is_time_machine_mode:
            pass
        else:
